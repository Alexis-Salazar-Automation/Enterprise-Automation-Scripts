...
//codigo para sacar acuse de semanas antes a la actual
/*function generarYEnviarAcusePDF(datos, destinatarioEmail) {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const plantillaSheet = ss.getSheetByName(HOJA_PLANTILLA_ACUSE);
    if (!plantillaSheet) throw new Error(`No se encontró la hoja plantilla "${HOJA_PLANTILLA_ACUSE}"`);
    ...
    try {
        ...
        // 🔹 CORRECCIÓN: semana del año - 1
        let semanaDelAnio = parseInt(
            Utilities.formatDate(hoy, Session.getScriptTimeZone(), "w"),
            10
        );
        semanaDelAnio = Math.max(1, semanaDelAnio - 1);
        ...
        const datosTabla = datos.convenios.map((convenio, index) => {
            ...
            const fechaEntregaStr = convenio.fechaEntrega
                ? Utilities.formatDate(new Date(convenio.fechaEntrega), timeZone, "dd/MM/yyyy")
                : "";
            const fechaRecepcionStr = convenio.fechaRecibo
                ? Utilities.formatDate(new Date(convenio.fechaRecibo), timeZone, "dd/MM/yyyy")
                : "";
        ...
        if (datosTabla.length > 0) {
            ...
            for (let i = 0; i < datosTabla.length; i++) {
                if ((i + 1) % 2 === 0) {
                    tempSheet
                        .getRange(tablaInicioFila + i, 2, 1, datosTabla[0].length)
                        .setBackground(colorFilaAlterna);
                }
            }
            ...
            tempSheet.getRange(filaInicioFirmas, labelStartCol, 1, labelNumCols)
                .merge().setValue("NOMBRE DE QUIEN RECIBE:")
                .setFontWeight("bold").setHorizontalAlignment("left").setVerticalAlignment("middle");
            tempSheet.getRange(filaInicioFirmas, lineStartCol, 1, lineNumCols)
                .merge().setBorder(null, null, true, null, null, null, "#000000", SpreadsheetApp.BorderStyle.SOLID);

            tempSheet.getRange(filaInicioFirmas + 2, labelStartCol, 1, labelNumCols)
                .merge().setValue("PUESTO:")
                .setFontWeight("bold").setHorizontalAlignment("left").setVerticalAlignment("middle");
            tempSheet.getRange(filaInicioFirmas + 2, lineStartCol, 1, lineNumCols)
                .merge().setBorder(null, null, true, null, null, null, "#000000", SpreadsheetApp.BorderStyle.SOLID);

            tempSheet.getRange(filaInicioFirmas + 4, labelStartCol, 1, labelNumCols)
                .merge().setValue("ÁREA:")
                .setFontWeight("bold").setHorizontalAlignment("left").setVerticalAlignment("middle");
            tempSheet.getRange(filaInicioFirmas + 4, lineStartCol, 1, lineNumCols)
                .merge().setBorder(null, null, true, null, null, null, "#000000", SpreadsheetApp.BorderStyle.SOLID);

            tempSheet.getRange(filaInicioFirmas + 6, labelStartCol, 1, labelNumCols)
                .merge().setValue("FECHA Y FIRMA:")
                .setFontWeight("bold").setHorizontalAlignment("left").setVerticalAlignment("middle");
            tempSheet.getRange(filaInicioFirmas + 6, lineStartCol, 1, lineNumCols)
                .merge().setBorder(null, null, true, null, null, null, "#000000", SpreadsheetApp.BorderStyle.SOLID);

            tempSheet.setColumnWidth(2, 20);
            tempSheet.setColumnWidth(3, 30);
            tempSheet.setColumnWidth(4, 130);
            tempSheet.setColumnWidth(5, 170);
            tempSheet.setColumnWidth(6, 30);
            tempSheet.setColumnWidth(7, 125);
            tempSheet.setColumnWidth(8, 125);
            tempSheet.setColumnWidth(9, 60);
        }
        ...
        const url_ext =
            ...
            '&sheetnames=false&printtitle=false&gridlines=false&gid=' + sheetId;
        const url = `https://docs.google.com/spreadsheets/d/${ssId}/${url_ext}`;
        ...
        if (response.getResponseCode() !== 200) {
            throw new Error(`El servidor de Google falló al intentar generar el PDF (código ${response.getResponseCode()}).`);
        }
        ...
        const listaFoliosHtml = '<ul>' + datos.convenios.map(c => `<li>${c.folio}</li>`).join('') + '</ul>';
        const cuerpoHtml =
            `<p>Hola, se adjunta el acuse en formato PDF con el resumen de los <b>${datos.convenios.length} convenios</b> ` +
            `registrados por <b>${datos.nombreAnalista}</b>. Los folios incluidos son:</p>` +
            `${listaFoliosHtml}<p><i>Este es un correo generado automáticamente.</i></p>`;

        MailApp.sendEmail({
            to: destinatarioEmail,
            cc: CORREOS_SUPERVISION_CC.join(','),
            subject: asunto,
            htmlBody: cuerpoHtml,
            attachments: [pdfBlob]
        });
...
/*function actualizarFechaCierreConcentrado() {
  ...
  if (!hojaVencidos || !hojaConcentrado) {
    Logger.log('Advertencia: No se encontraron "Convenios Vencidos" o "Concentrado" para actualizar fecha de cierre.');
    return;
  }
  
  // 1. Crear un mapa de Folio -> Fecha Cierre desde la hoja de Vencidos
  const datosVencidos = hojaVencidos.getDataRange().getValues();
  const encabezadosVencidos = datosVencidos.shift();
  const idxFolioVencido = encabezadosVencidos.indexOf('Folio');
  const idxFechaCierreVencido = encabezadosVencidos.indexOf('Fecha Cierre');
  
  if (idxFolioVencido === -1 || idxFechaCierreVencido === -1) {
    Logger.log('Advertencia: Faltan columnas "Folio" o "Fecha Cierre" en "Convenios Vencidos".');
    return;
  }
  
  const mapaFechasCierre = new Map();
  datosVencidos.forEach(fila => {
    const folio = fila[idxFolioVencido];
    const fechaCierre = fila[idxFechaCierreVencido];
    if (folio && fechaCierre) {
      mapaFechasCierre.set(folio, fechaCierre);
    }
  });

  if (mapaFechasCierre.size === 0) return; // No hay nada que actualizar

  // 2. Recorrer el Concentrado y preparar la actualización
  ...
  if (idxFolioConcentrado === -1 || idxFechaCierreConcentrado === -1) {
    Logger.log('Advertencia: Faltan columnas "Folio" o "Fecha Cierre" en "Concentrado".');
    return;
  }

  const nuevosValoresFecha = [];
  let huboCambios = false;
  // Recorremos los datos para construir solo las actualizaciones necesarias
  datosConcentrado.slice(1).forEach((fila, index) => {
    const folioActual = fila[idxFolioConcentrado];
    const fechaActual = fila[idxFechaCierreConcentrado];
    if (mapaFechasCierre.has(folioActual) && !fechaActual) { // Solo actualiza si la celda está vacía
      const nuevaFecha = mapaFechasCierre.get(folioActual);
      hojaConcentrado.getRange(index + 2, idxFechaCierreConcentrado + 1).setValue(nuevaFecha);
      huboCambios = true;
    }
  });
  
  if (huboCambios) {
      Logger.log('Se actualizaron las fechas de cierre en el Concentrado.');
  }
}*/
...
/*function actualizacionNegociacionesVencidas() {
  ...
  if (!hojaNegociaciones) {
    return { resumen: 'Error: No se encontró la hoja "Negociacion-PP".', incompletos: [], completados: [] };
  }

  // Usamos el método preciso para contar solo las filas con datos en la columna A
  const lastRow = hojaNegociaciones.getRange("A:A").getValues().filter(String).length;
  if (lastRow < 2) {
    return { resumen: 'No se encontraron negociaciones para procesar.', incompletos: [], completados: [] };
  }
  ...
  if ([idx.vencimiento, idx.morosidadCartera, idx.cierre].includes(-1)) {
    const columnaFaltante = Object.keys(idx).find(key => idx[key] === -1);
  ...
  // Creamos una copia de la columna de cierre para modificarla en memoria
  let columnaCierreActualizada = datos.map(fila => [fila[idx.cierre]]);

  // Iteramos sobre los datos (empezando en la fila 1 para saltar encabezados)
  for (let i = 1; i < datos.length; i++) {
    const fila = datos[i];
    const cierreActual = fila[idx.cierre];

    // --- LÓGICA CLAVE: Solo procesamos las filas que aún no tienen un estatus de cierre ---
    if (cierreActual === '') {
      const diasParaVencer = fila[idx.vencimiento];
      const morosidad = fila[idx.morosidadCartera];
      let nuevoEstatus = '';

      // Condición 1: Vencido por tiempo
      if (typeof diasParaVencer === 'number' && diasParaVencer < 0) {
        nuevoEstatus = 'Incompleto';
        incompletosParaReporte.push([fila[idx.folio], fila[idx.propietario], fila[idx.lote]]);
      } 
      // Condición 2: Liquidado
      else if (morosidad === 0) {
        nuevoEstatus = 'Completado';
        completadosParaReporte.push([fila[idx.folio], fila[idx.propietario], fila[idx.lote]]);
      }
      
      // Si se encontró un nuevo estatus, lo actualizamos en nuestra copia de la columna
      if (nuevoEstatus !== '') {
  ...
  // Si hubo cambios, escribimos TODA la columna de cierre de una sola vez para ser eficientes
  if (filasActualizadas > 0) {
    hojaNegociaciones.getRange(1, idx.cierre + 1, columnaCierreActualizada.length, 1).setValues(columnaCierreActualizada);
  }

  // Devolvemos el objeto completo con el resumen y las listas para el reporte
  return {
...
/*function actualizacionNegociacionesVencidas() {
  ...
  const idx = {
    ...
    morosidadCartera: encabezados.indexOf('Morosidad (Cartera)'),
    cierre: encabezados.indexOf('Cierre NN-PP'),
    fechaCierre: encabezados.indexOf('Fecha Cierre'), // <-- NUEVO: Se busca la nueva columna
    folio: encabezados.indexOf('Folio'),
    propietario: encabezados.indexOf('Nombre Propietario'),
    lote: encabezados.indexOf('Lote Cartera')
  };
  ...
  if (Object.values(idx).some(i => i === -1)) {
    const columnaFaltante = Object.keys(idx).find(key => idx[key] === -1);
    return { resumen: `Error en "Negociacion-PP": No se encontró la columna "${columnaFaltante}".`, incompletos: [], completados: [] };
  }
  ...
  let columnaCierreActualizada = datos.map(fila => [fila[idx.cierre]]);
  let columnaFechaCierreActualizada = datos.map(fila => [fila[idx.fechaCierre]]); // <-- NUEVO: Se prepara la columna de fecha
  ...
  for (let i = 1; i < datos.length; i++) {
    ...
    if (fila[idx.cierre] === '') {
      ...
      else if (morosidad === 0 || morosidad === '-' || morosidad === 1) { // Ampliamos la regla aquí también
        nuevoEstatus = 'Completado';
        completadosParaReporte.push([fila[idx.folio], fila[idx.propietario], fila[idx.lote]]);
      }
      ...
      if (nuevoEstatus !== '') {
        columnaCierreActualizada[i][0] = nuevoEstatus;
        columnaFechaCierreActualizada[i][0] = fechaDeHoy; // <-- NUEVO: Se asigna la fecha
        filasActualizadas++;
      }
    }
  }
  ...
  if (filasActualizadas > 0) {
    // Escribimos ambas columnas actualizadas
    hojaNegociaciones.getRange(1, idx.cierre + 1, columnaCierreActualizada.length, 1).setValues(columnaCierreActualizada);
...
/*function enviarRecordatoriosDeVencimiento() {
  ...
  datos.forEach((fila, index) => {
    ...
    if (typeof diasParaVencer === 'number' && diasParaVencer >= 0 && diasParaVencer <= 7 && !fila[indices.recordatorio]) {
      ...
      // --- INICIO DE LA CORRECCIÓN ---
      // Se restaura el cuerpo del correo a su versión completa y detallada.
      const cuerpoHtml = `
  ...
  if (datosParaReporte.length > 0) {
    return {
      resumen: `Se enviaron ${datosParaReporte.length} nuevos recordatorios de vencimiento.`,
      datos: datosParaReporte
    };
  } else {
    return { resumen: 'No se encontraron nuevos convenios por notificar.', datos: [] };
  }
}*/
...
/*function recordatorioNegociacionesPP() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const hojaNegociaciones = ss.getSheetByName('Negociacion-PP'); // Lee de la hoja correcta
  if (!hojaNegociaciones) {
  ...
  // Mapeo de columnas importantes para la hoja "Negociacion-PP"
  const indices = { 
    vencimiento: encabezados.indexOf('Vencimiento'), 
    correoAnalista: encabezados.indexOf('Correo Analista'), // Columna para el destinatario
    lote: encabezados.indexOf('Lote Cartera'), 
    folio: encabezados.indexOf('Folio'), 
    propietario: encabezados.indexOf('Nombre Propietario'), 
    recordatorio: encabezados.indexOf('Recordatorio'), // Columna para marcar el envío
    fechaFin: encabezados.indexOf('Fecha Fin')
  };
  ...
  datos.forEach((fila, index) => {
    ...
    // La condición es la misma: valor numérico entre 0 y 3, y que no se haya enviado un recordatorio
    if (typeof diasParaVencer === 'number' && diasParaVencer >= 0 && diasParaVencer <= 3 && !fila[indices.recordatorio]) {
      ...
      if (!correoDestinatario) return; // Si no hay correo, no hace nada
      
      // Se pueden añadir correos en copia si es necesario en el futuro
      ...
      // Cuerpo del correo adaptado para Negociaciones y Promesas de Pago
      const cuerpoHtml = `
        <p>Hola,</p>
      ...
      // Marca en la hoja que el recordatorio fue enviado para no duplicar
      hojaNegociaciones.getRange(index + 2, indices.recordatorio + 1).setValue(fechaActual);
      
      // Guarda los datos para incluirlos en el reporte diario automático
  ...
  if (datosParaReporte.length > 0) {
    ...
    return { resumen: 'No se encontraron nuevas Negociaciones/PP por notificar.', datos: [] };
  }
}*/
...
/*1function generarReporteMensual(fechas) {
  ...
  if (!hojaReporte) {
    hojaReporte = ss.insertSheet(nombreHojaReporte);
  }
  ...
  if (idx.fechaCierre === -1 || idx.estatus === -1) { throw new Error('Faltan las columnas "Fecha Cierre" o "Estatus General".'); }
  ...
  datos.forEach(fila => {
    const folio = fila[idx.folio], fechaCierre = fila[idx.fechaCierre], estatus = fila[idx.estatus];
    if (String(folio).startsWith('CONV-') && fechaCierre instanceof Date && fechaCierre >= fechaInicio && fechaCierre <= fechaFin) {
      ...
      if (estatus === 'Completado') resultadosPorMes[mesKey].exitosos++;
      else if (estatus === 'Incompleto') resultadosPorMes[mesKey].incumplidos++;
    }
  });
  
  hojaReporte.getRange("B2:H2").merge().setValue("Reporte Mensual de Convenios Concluidos").setFontSize(16).setFontWeight('bold').setHorizontalAlignment('center');
  ...
  const datosTabla = [['Mes', 'Exitosos', 'Incumplidos', 'Total Cierres', '% Exitosos', '% Incumplidos', 'Total %']];
  datosAgrupados.forEach(mes => {
    const total = mes.exitosos + mes.incumplidos;
    const porcExito = (total > 0) ? mes.exitosos / total : 0;
    const porcIncumplido = (total > 0) ? mes.incumplidos / total : 0;
    datosTabla.push([ mes.nombre, mes.exitosos, mes.incumplidos, total, porcExito, porcIncumplido, porcExito + porcIncumplido ]);
  });
  ...
  if (datosTabla.length > 1) {
    ...
    const efectividadTotal = (granTotal > 0) ? totalExitosos / granTotal : 0;
    const incumplimientoTotal = (granTotal > 0) ? totalIncumplidos / granTotal : 0;
    datosTabla.push(['TOTALES', totalExitosos, totalIncumplidos, granTotal, efectividadTotal, incumplimientoTotal, efectividadTotal + incumplimientoTotal]);

    // ESCRITURA Y FORMATEO
    const rangoTabla = hojaReporte.getRange(5, 2, datosTabla.length, 7);
    rangoTabla.setValues(datosTabla);

    rangoTabla.setBorder(true, true, true, true, true, true, '#B7B7B7', SpreadsheetApp.BorderStyle.SOLID);
    hojaReporte.getRange("B5:H5").setBackground('#4A86E8').setFontColor('white').setFontWeight('bold');
    const filaTotales = 5 + datosTabla.length - 1;
    hojaReporte.getRange(`B${filaTotales}:H${filaTotales}`).setBackground('#CFE2F3').setFontWeight('bold');
    
    hojaReporte.getRange(6, 3, datosTabla.length - 1, 6).setHorizontalAlignment('center');
    hojaReporte.getRange(6, 6, datosTabla.length - 1, 3).setNumberFormat('0.00%');
    hojaReporte.autoResizeColumns(2, 7);
    
    // CREACIÓN DE GRÁFICOS
    const rangoDatosGraficoBarras = hojaReporte.getRange("B5:D" + (4 + datosTabla.length - 1)); // No incluye la fila de totales
    const barChart = hojaReporte.newChart().asColumnChart()
        .addRange(rangoDatosGraficoBarras).setMergeStrategy(Charts.ChartMergeStrategy.MERGE_COLUMNS)
        .setNumHeaders(1).setOption('title', 'Cierres de Convenio por Mes')
        .setOption('colors', ['#6AA84F', '#CC0000']).setOption('isStacked', 'true')
        .setOption('legend', { position: 'top' })
        .setOption('series', { 0: { dataLabel: 'value' }, 1: { dataLabel: 'value' } })
        .setPosition(5 + datosTabla.length + 2, 2, 0, 0).build();
    hojaReporte.insertChart(barChart);

    const rangoResumenPie = hojaReporte.getRange(2, 9, 3, 2); // Coloca los datos para el pastel en I2:J4
    rangoResumenPie.setValues([['Tipo', 'Total'], ['Exitosos', totalExitosos], ['Incumplidos', totalIncumplidos]]);
    ...
    const pieChart = hojaReporte.newChart().asPieChart()
        ...
        .setNumHeaders(1).setOption('title', 'Distribución Total del Período (%)')
        .setOption('colors', ['#6AA84F', '#CC0000']).setOption('pieHole', 0.4)
        .setOption('legend', { position: 'labeled' }).setOption('pieSliceText', 'value')
        .setPosition(5 + datosTabla.length + 2, 8, 0, 0).build();
    hojaReporte.insertChart(pieChart);
    hojaReporte.hideColumns(9, 2); // Oculta las columnas I y J
    
  } else {
    hojaReporte.getRange("B5").setValue("No se encontraron convenios concluidos en el período seleccionado.").setFontStyle('italic');
  }
